...
def get_location(soup):
    try:
        location = soup.find('span', itemprop="address").text
    except:
        print "\n*****************"
        print "Check get_location"
    
    full_address, address, location = featurizing_locations(location)
    print full_address, address, location
...
def main():
    ...
    all_companies = all_titles = all_logos = all_descriptions = \
    all_job_links = all_logo_links = all_company_links = all_countries \
    = all_full_address = all_scraped_address = all_locations = \
    all_country = []

    for job in jobs:
        for country in countries:
            for i in xrange(100000):
                filename = 'data/html_sample/linkedin_' + str.replace(job, ' ', '_')\
                ...
                all_full_address.append(full_address)
    ...
    'logo' : all_logos, 'company_link' : all_company_links, 'country' : all_country})
    print df
